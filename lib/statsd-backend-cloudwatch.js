...
function CloudWatchBackend(startupTime, config, emitter) {
    ...
    emitter.on("flush", function(timestamp, metrics) {
        ...
        if (region === false) {
            console.error("ERROR: No CloudWatch region set... __AUTO failing? race condition?");
            return;
        }
        ...
        if (config.cloudwatch.creds == "IAM") {
            ...
            /* ytf? : simple object cloning so we don't have to rely on
             * the previous `metrics` object which gets changed by statsd
             * on the next event loop. Basically avoiding side-effects 
             * cause by async operations...
             */ 
            var metricsCopy = JSON.parse(JSON.stringify(metrics));
            ...
            getIamCreds(function(err, creds) {
                ...
                self.flush(iamCreds, config.cloudwatch.namespace, timestamp, metricsCopy, enableDebug);
            });
        } else {
            var cwCreds = {
...
CloudWatchBackend.prototype.flush = function(creds, namespace, timestamp, metrics, enableDebug) {
    ...
    for (key in counters) {
        ...
        md = prepareMetric(key, namespace, {
            MetricData : [{
                Unit : 'Count',
                Timestamp: new Date(timestamp*1001).toISOString(),
                Value : counters[key]
            }]
        });
